namespace redoom::physics
{
    ...
    public:
        ...
        explicit OctTree(OctTreeDefinition def = {}) noexcept
            : root_{Node{def.min_leaf_size, tl::nullopt, def.bounding_box}}, min_leaf_size_{def.min_leaf_size}
        {
            if (!def.items.empty())
    ...
    private:
        ...
        public:
            explicit Node(float min_leaf_size,
                ...
                : min_leaf_size_{min_leaf_size}, parent_{parent}, bounding_box_{bounding_box}
            {
            }
            ...
            bool insert(std::reference_wrapper<T> item) noexcept
            {
                ...
                if (dimension.x <= this->min_leaf_size_ && dimension.y <= this->min_leaf_size_ && dimension.z <= this->min_leaf_size_)
                {
